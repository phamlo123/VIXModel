...
connection = mysql.connector.connect(user='root', password='Philong1903',
...
def somethingelse(date):
    statement = 'SELECT futureRealizedVariance FROM dateAndPriceFeatures WHERE date_from_1996 = {d} LIMIT 1'.format(
        d=date)
    cursor.execute(statement)
